...
class userDirectoryApp {
		constructor(){
			...
			this.getUserList().then((value) => {
				...
				this.userList.forEach((val) => {
					var taskComponent = document.createElement('contact-item');
		...
		getUserList() {
			return new Promise((resolve,reject) => {
					...
					HTTP.open("GET","https://api.myjson.com/bins/e5mq0");
					HTTP.send();
		...
		searchHandler(e){
			...
			var documentFragment = document.createDocumentFragment();
			var filteredList = this.taskScope.userList.filter(function(item){
...
customElements.define('search-component',
	class extends HTMLElement {
		...
		searchHandler(e){
			this.taskScope.setAttribute("data-value",this.taskScope.searchComponent.value);
		}	
	}
);	
